-- Migration: 20260923000000_shop_search_and_filters.sql
-- Description: Extended coffee shop nearby search RPC supporting search by name, minimum rating filter, and sorting by distance or rating

-- 1. Drop existing 5-parameter function signature to prevent overload ambiguity
DROP FUNCTION IF EXISTS get_nearby_shops(DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, INTEGER, INTEGER);

-- 2. Create updated get_nearby_shops SQL RPC with search, filter, and sort support
...
BEGIN
    ...
    IF sort_by IS NULL OR sort_by <> 'rating' THEN
        sort_by := 'distance';
    END IF;
    ...
    ORDER BY
        CASE WHEN sort_by = 'rating' THEN calc.rating END DESC NULLS LAST,
        calc.distance_meters ASC,
        calc.id ASC
    LIMIT result_limit
